...
const CheckoutPage: React.FC = () => {
  ...
  const { plans, loading: subscriptionLoading } = useAppSelector((state) => state.subscription);
  const { couponValidation, loading: billingLoading } = useAppSelector((state) => state.billing);
  ...
  const handleCouponValidation = async () => {
    ...
    try {
      await dispatch(validateCoupon(formData.couponCode)).unwrap();
  ...
  const calculateTotal = () => {
    if (!selectedPlan) return { subtotal: 0, tax: 0, discount: 0, total: 0 };
    
    const basePrice = billingCycle === 'yearly' ? selectedPlan.price_yearly : selectedPlan.price_monthly;
    const subtotal = basePrice;
    let discount = 0;
    
    if (couponApplied && couponValidation.coupon) {
      if (couponValidation.coupon.discount_type === 'percentage') {
        discount = (subtotal * couponValidation.coupon.discount_value) / 100;
      } else {
        discount = couponValidation.coupon.discount_value;
      }
    }
    
    const discountedAmount = subtotal - discount;
    const tax = discountedAmount * 0.18; // 18% KDV
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const subscriptionData = {
        ...
        payment_method_id: 'pm_mock_' + Date.now(),
        coupon_code: couponApplied ? formData.couponCode : undefined,
      };
      ...
      toast({
        ...
        description: 'Hesabınız aktif edildi. Dashboard\'a yönlendiriliyorsunuz.',
        status: 'success',
        duration: 5000,
        isClosable: true,
      });
      ...
      <Box bg={bg} minH="100vh" py={20}>
        <Container maxW="4xl">
          <VStack spacing={8}>
            ...
            <Button onClick={() => navigate('/pricing')} colorScheme="blue">
    ...
    <Box bg={bg} minH="100vh" py={8}>
      <Container maxW="6xl">
        <VStack spacing={8}>
          ...
          <Breadcrumb w="full">
            <BreadcrumbItem>
              <BreadcrumbLink onClick={() => navigate('/pricing')} cursor="pointer">
                Fiyatlandırma
              </BreadcrumbLink>
            </BreadcrumbItem>
